function [best_fitness, best_solution, fitness_history] = de_rastrigin_current_to_rand()
% DE/current-to-rand/1/bin - Thuật toán Tiến hóa Sai phân sử dụng chiến lược current-to-rand
% Giải bài toán Rastrigin.
    ...
    fprintf('DE/current-to-rand/1/bin | n=%d, N=%d, MAX_GEN=%d\n', N_DIM, POP_SIZE, MAX_GEN);
    disp('--- Bat dau toi uu ---');
    ...
    for gen = 1:MAX_GEN
        ...
        for i = 1:POP_SIZE
            ...
            % 4a. Đột biến (Mutation: DE/current-to-rand/1)
            
            % Chọn ngẫu nhiên 3 chỉ số khác nhau
            indices = 1:POP_SIZE;
            indices(i) = []; % Loại bỏ cá thể mục tiêu (i)
            
            % Phải chọn 3 cá thể r1, r2, r3 khác nhau và khác i
            if POP_SIZE < 4
                 % Xử lý trường hợp N quá nhỏ (ví dụ N=3), nên tránh
            ...
            % Vectơ Đột biến V: V = I_i + rand * (I_r1 - I_i) + F * (I_r2 - I_r3)
            % rand() trong MATLAB trả về số trong [0, 1]
            V = Target + rand() * (Pop(r1, :) - Target) + F * (Pop(r2, :) - Pop(r3, :));
